...
import org.workswap.location.datasource.repository.NotificationRepository;
import org.workswap.location.dto.FullNotificationDTO;
import org.workswap.location.dto.NotificationDTO;
import org.workswap.location.enums.NotificationType;
import org.workswap.location.services.NotificationCommandService;
...
public class NotificationCommandServiceImpl implements NotificationCommandService {
    ...
    public FullNotificationDTO saveChatNotification(Long userId, NotificationDTO dto) {
    ...
    public void sendNotificationToTelegram(User user, NotificationDTO dto) {
        try {
            ...
            requestBody.put("message", dto.message());
    ...
    public void sendNotification(User receiver, NotificationDTO notificationDto) {
        ...
        /* FullNotificationDTO fullNotification = saveChatNotification(receiver.getEmail(), notificationDto);
        ...
        if (!isUserOnline(receiver)) {
            logger.debug("Пользователь {} офлайн, отправляем уведомление в телеграм", receiver.getEmail());
            sendNotificationToTelegram(receiver, notificationDto);
        } else {
            logger.debug("Пользователь {} онлайн, отправляем уведомление", receiverOpenId);
    ...
    private boolean isUserOnline(User user) {
        ...
        if (openId == null) {
            throw new IllegalStateException("У проверяемого пользователя нет почты!");
        }

        return simpUserRegistry.getUser(openId) != null;
    }
}
